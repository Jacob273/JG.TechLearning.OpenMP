...
namespace SierpinskiOpenGL
{
	...
	public:
		...
		void StartDrawingSierpinski(Triangle* firstTriangle, int level)
		{
			...
			if (level > 0)
			{
				...
				RecursiveDrawingSubTriangles(second, level);

			}
		}

		void RecursiveDrawingSubTriangles(Triangle* triangle, int level)
		{
			_id = ++counter;
			...
			triangle->PrintMyVertices();
			DrawTriangle(triangle, r, g, b);
			if (level > 0)
			{
				GeometryHelper helper = GeometryHelper();

				GLfloat* a1 = helper.GenerateVertices((triangle->A[0] + triangle->C[0]) / 2, (triangle->A[1] + triangle->C[1]) / 2);
				GLfloat* b1 = helper.GenerateVertices(a1[0] + (triangle->C[0] - triangle->B[0]) / 2, a1[1] + (triangle->C[1] - triangle->B[1]) / 2);
				GLfloat* c1 = helper.GenerateVertices(a1[0] + (triangle->A[0] - triangle->C[0]) / 2, a1[1] + (triangle->A[1] - triangle->B[1]) / 2);
				Triangle* left = new Triangle(a1, b1, c1);

				GLfloat* a2 = helper.GenerateVertices((triangle->B[0] + triangle->C[0]) / 2, (triangle->A[1] + triangle->C[1]) / 2);
				GLfloat* b2 = helper.GenerateVertices(a2[0] + (triangle->C[0] + triangle->B[0]) / 2, a2[1]);
				GLfloat* c2 = helper.GenerateVertices(a2[0] + (triangle->C[0] + triangle->B[0]) / 2, triangle->C[1]);
				Triangle* right = new Triangle(a2, b2, c2);

				RecursiveDrawingSubTriangles(left, level - 1);
				RecursiveDrawingSubTriangles(right, level - 1);
			}
		}
		...
		void Trampoline_MainDrawing()
		{
			...
			int level = 2;
			StartDrawingSierpinski(main, level);
	...
	DrawingObject drawObj = DrawingObject();
